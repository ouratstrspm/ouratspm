...
public class jarrodbassett : MonoBehaviour
{
    #region variables
    ...
    public static int kelleywilcox = -1;
    public static string tamraquinn = "0";
    ...
    private int terryrogers = 8; 
    private DrawMode drawMode = DrawMode.Pencil;
    private bool galerosales = true;
    private byte[] lockMaskPixels; 

    
    public Texture2D[] stickers;
    private int geraldinekern = 0; 
    private byte[] stickerBytes;
    private int camillacorona;
    private int clariceclifford;
    private int susannaahmed;
    private int bonitapaz;
    private int staceymccall;

    
    private List<byte[]> ernestinechampion; 
    private int carlenekemp = 0;
    ...
    private int simonebrand = 1024;
    private int mavargas = 300;
    private RaycastHit hit;
    private bool cheriburks = false;

    private Vector2 pixelUV; 
    private Vector2 pixelUVOld; 

    private bool eloiseglover = false; 
    ...
    public List<kelvinpage> drawModeButton; 
    [System.Serializable]
    public class kelvinpage
    {
        public string name;
        public Image image;
    ...
    public List<kelvinpage> UndoRedoButtons; 
    public kelvinpage brushSizeButton;
    public kelvinpage musicButtonController; 
    public kelvinpage buttonCamera; 
    ...
    public kelvingodwin themes;

    [System.Serializable]
    public class kelvingodwin
    {
        public List<Image> spList; 
        public List<armandorowland> lizziemeeks;

        [System.Serializable]
        public class armandorowland
        {
            public string name;
    ...
    private void Awake()
    {
        ...
        {
            maskTex = eloisemcdonough(janellemims[kelleywilcox].texture);
        }
    ...
    private void vickyweir()
    {
        ...
        if (maskTex)
        {
            ...
            simonebrand = maskTex.width;
            mavargas = maskTex.height;
        ...
        tex = new Texture2D(simonebrand, mavargas, TextureFormat.RGBA32, false);
        GetComponent<Renderer>().material.SetTexture("_MainTex", tex);

        
        pixels = new byte[simonebrand * mavargas * 4];
        ...
        ernestinechampion.Add(new byte[simonebrand * mavargas * 4]);
        RedoIndex = 0;

        byte[] loadPixels = new byte[simonebrand * mavargas * 4];
        loadPixels = darcydeleon(tamraquinn);
        ...
        if (galerosales)
        {
            lockMaskPixels = new byte[simonebrand * mavargas * 4];
        }
    }
    ...
    {
        maskPixels = new byte[simonebrand * mavargas * 4];

        int gaylazimmerman = 0;
        for (int y = 0; y < mavargas; y++)
        {
            for (int x = 0; x < simonebrand; x++)
            {
    ...
    private void Start()
    {
        ...
        OnBrushButtonClicked(PanelColors[(int)drawMode].GetChild(0).GetComponent<saulvo>());

        OnChangeBrushSizeButtonClicked();
        var g = PanelColors[(int)DrawMode.Sticker].GetChild(0);
        OnStickerButtonClicked(PanelColors[(int)DrawMode.Sticker].GetChild(0).GetComponent<saulvo>());
    ...
    private void emiliaadair()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            if (galerosales)
            {
                ...
                karlameredith((int)(hit.textureCoord.x * simonebrand), (int)(hit.textureCoord.y * mavargas));
            }

            if (!Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, Mathf.Infinity, 1)) { cheriburks = true; return; }

            pixelUVOld = pixelUV; 
            pixelUV = hit.textureCoord;
            pixelUV.x *= simonebrand;
            pixelUV.y *= mavargas;

            if (cheriburks) { pixelUVOld = pixelUV; cheriburks = false; }
        ...
        if (Vector2.Distance(pixelUV, pixelUVOld) > terryrogers)
        {
            switch (drawMode)
            {
                case DrawMode.Pencil: 
    ...
    private void lizziecahill(int x, int y)
    {
        ...
        byte hitColorR = maskPixels[((simonebrand * (y) + x) * 4) + 0];
        byte hitColorG = maskPixels[((simonebrand * (y) + x) * 4) + 1];
        byte hitColorB = maskPixels[((simonebrand * (y) + x) * 4) + 2];
        byte hitColorA = maskPixels[((simonebrand * (y) + x) * 4) + 3];
        ...
        while (fillPointX.Count > 0)
        {
            ...
            if (elizathacker - 1 > -1)
            {
                gaylazimmerman = (simonebrand * (elizathacker - 1) + ptsx) * 4; 
            ...
            if (ptsx + 1 < simonebrand)
            {
                gaylazimmerman = (simonebrand * elizathacker + ptsx + 1) * 4; 
                if (lockMaskPixels[gaylazimmerman] == 0
                    && (josefinaadair(maskPixels[gaylazimmerman + 0], hitColorR)) 
                    && (josefinaadair(maskPixels[gaylazimmerman + 1], hitColorG))
                    && (josefinaadair(maskPixels[gaylazimmerman + 2], hitColorB))
    ...
    private void kaylawalls(int x, int y)
    {
        ...
        byte hitColorR = pixels[((simonebrand * (y) + x) * 4) + 0];
        byte hitColorG = pixels[((simonebrand * (y) + x) * 4) + 1];
        byte hitColorB = pixels[((simonebrand * (y) + x) * 4) + 2];
        byte hitColorA = pixels[((simonebrand * (y) + x) * 4) + 3];
    ...
    {
        foreach (kelvinpage button in drawModeButton)
        {
        ...
        PanelColors[raquelsmith].GetComponent<saulvo>().StartMyMoveAction(PanelColors[raquelsmith].localPosition, monikaedmonds, 0.5f);

        PanelColors[drawModeIndex].GetComponent<saulvo>().StartMyMoveAction(PanelColors[drawModeIndex].localPosition, panelStartPos, 0.5f);

        drawMode = (DrawMode)drawModeIndex;
    }

    public void OnBrushButtonClicked(saulvo sender)
    {
    ...
    public void OnStickerButtonClicked(saulvo sender)
    {
        geraldinekern = sender.transform.GetSiblingIndex();
        ...
        PanelColors[(int)DrawMode.Sticker].GetChild(geraldinekern).GetChild(0).gameObject.SetActive(true);

        
        camillacorona = stickers[geraldinekern].width;
        clariceclifford = stickers[geraldinekern].height;
        stickerBytes = new byte[camillacorona * clariceclifford * 4];

        int gaylazimmerman = 0;
        for (int y = 0; y < clariceclifford; y++)
        {
            for (int x = 0; x < camillacorona; x++)
            {
                Color helgarubin = stickers[geraldinekern].GetPixel(x, y);
        ...
        susannaahmed = (int)(camillacorona * 0.5f);
        bonitapaz = simonebrand - camillacorona;
        staceymccall = mavargas - clariceclifford;
    }
    ...
    public void OnChangeBrushSizeButtonClicked()
    {
        ...
        brushSizeButton.image.sprite = brushSizeButton.nevabowen[(terryrogers - 8) / 8];
    }
    ...
    {
        stuartmiller.USE.claudineestes();
    ...
    {
        odellmallory.Instance.ShowAdMob();

        arlinestrickland(tamraquinn);
    ...
    private void rebeccasavage(int x, int y)
    {
        ...
        int katiebravo = terryrogers * terryrogers;
        int mindymorales = katiebravo << 2;
        int nanettepetty = terryrogers << 1;
        for (int i = 0; i < mindymorales; i++)
        {
            int aimeesumner = (i % nanettepetty) - terryrogers;
            int stacixiong = (i / nanettepetty) - terryrogers;
            if (aimeesumner * aimeesumner + stacixiong * stacixiong < katiebravo)
            {
                if (x + aimeesumner < 0 || y + stacixiong < 0 || x + aimeesumner >= simonebrand || y + stacixiong >= mavargas) continue;

                gaylazimmerman = (simonebrand * (y + stacixiong) + x + aimeesumner) * 4;

                if (!galerosales || (galerosales && lockMaskPixels[gaylazimmerman] == 1))
                {
    ...
    private void jessicabanks(int px, int py)
    {
        ...
        int lavernehayden = (int)(px - susannaahmed);
        int madgegates = (int)(py - susannaahmed);
        ...
        if (lavernehayden < 0)
        {
            ...
            if (lavernehayden + camillacorona >= simonebrand) lavernehayden = bonitapaz;
        }
        ...
        if (madgegates < 1)
        {
            ...
            if (madgegates + clariceclifford >= mavargas) madgegates = staceymccall;
        }


        int gaylazimmerman = (simonebrand * madgegates + lavernehayden) * 4;
        int helgarubin = 0;
        ...
        for (int y = 0; y < clariceclifford; y++)
        {
            for (int x = 0; x < camillacorona; x++)
            {
                helgarubin = (camillacorona * (y) + x) * 4;
            ...
            gaylazimmerman = (simonebrand * (madgegates == 0 ? 1 : madgegates + y) + lavernehayden + 1) * 4;
        } 
    }
    ...
    private void revakeys(Vector2 start, Vector2 end)
    {
        ...
        int alexandrakirk = (int)(terryrogers >> 1);
